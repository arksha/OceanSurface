		...
		for (int j = 0; j < M; j++)
		{
			...
			for (int n = 0; n < N; n++)
				for (int m = 0; m < M; m++)
				{
					...
					// This will make the sign get flipped, like the result in FFT. Because (+N/2) will lead to e^i which flips the sign of the real part.

					//height += (value_h_twiddle[index] * exp(complex<float32>(0.0, dot(
					//	vec2(2 * M_PI * n / L_x, 2 * M_PI * m / L_z),							// k_vec
					//	vec2(i * L_x / N, j*L_z / M))))).real();								// x_vec

					height += (value_h_twiddle[index] * exp(complex<float32>(0.0, dot(
						vec2(2 * M_PI * (n - N / 2) / L_x, 2 * M_PI * (m - M / 2) / L_z),		// k_vec
						vec2((i - N / 2) * L_x / N, (j - M / 2) * L_z / M))))).real();			// x_vec
